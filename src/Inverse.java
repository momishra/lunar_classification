...
public class Inverse {
	...
	public DenseMatrix getMPInverse() throws NotConvergedException{
		...
		double []s = svd.getS();
		int sn = s.length; //sn = 20
		for (int i = 0; i < sn; i++) {
			s[i] = Math.sqrt(s[i]);
		}
		
		DenseMatrix S1 = (DenseMatrix) Matrices.random(m, sn); //5000*20
		S1.zero();
		DenseMatrix S2 = (DenseMatrix) Matrices.random(sn, n); //20*20
		S2.zero();
		for (int i = 0; i < s.length; i++) {
			S1.set(i, i, s[i]);
			S2.set(i, i, s[i]);
		}
		DenseMatrix C = new DenseMatrix(m,sn); //5000*20
		U.mult(S1, C);
		DenseMatrix D = new DenseMatrix(sn,n); //20*20
		S2.mult(Vt,D);
		...
		DT.mult(invDD, DDD); //if rows are linearly independent, this is how we calculate psudeoInverse
		...
		invCC.mult(CT, CCC); //if columns are linearly independent, this is how we calculate psuedoInverse
